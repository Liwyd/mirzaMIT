...
if ($text == $textbotlang['Admin']['Usertest']['setvolumeusertest']) {
    ...
    if (!ctype_digit($text) || intval($text) < 100) {
        sendmessage($from_id, "❌ حجم نامعتبر است. حداقل حجم مجاز اکانت تست 100 مگابایت است (حجم نامحدود پشتیبانی نمی‌شود).", $backadmin, 'HTML');
        return;
    }
...
if ($text == $textbotlang['Admin']['keyboardadmin']['manage_panel']) {
    ...
    step('home', $from_id);
} elseif ($text == $textbotlang['Admin']['managepanel']['keyboardpanel']['editmitcred']) {
    ...
    update("marzban_panel", $field, $text, "name_panel", $user['Processing_value']);
    $typepanel = select("marzban_panel", "*", "name_panel", $user['Processing_value'], "select");
    outtypepanel($typepanel['type'], "✅ اطلاعات با موفقیت بروزرسانی شد.");
    step('home', $from_id);
