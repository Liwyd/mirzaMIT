...
try {
    ...
    if (!$table_exists) {
        ...
        $Check_filde = $connect->query("SHOW COLUMNS FROM marzban_panel LIKE 'marzban_url_direct'");
        if (mysqli_num_rows($Check_filde) != 1) {
            $connect->query("ALTER TABLE marzban_panel ADD marzban_url_direct VARCHAR(500) DEFAULT NULL");
            echo "The marzban_url_direct field was added ✅";
        }
        $Check_filde = $connect->query("SHOW COLUMNS FROM marzban_panel LIKE 'marzban_username_direct'");
        if (mysqli_num_rows($Check_filde) != 1) {
            $connect->query("ALTER TABLE marzban_panel ADD marzban_username_direct VARCHAR(200) DEFAULT NULL");
            echo "The marzban_username_direct field was added ✅";
        }
        $Check_filde = $connect->query("SHOW COLUMNS FROM marzban_panel LIKE 'marzban_password_direct'");
        if (mysqli_num_rows($Check_filde) != 1) {
            $connect->query("ALTER TABLE marzban_panel ADD marzban_password_direct VARCHAR(200) DEFAULT NULL");
            echo "The marzban_password_direct field was added ✅";
        }
